...
use crate::{HTTP_CLIENT, utils::deserialize_string_to_usize};
...
pub async fn get_suggestions(term: &str) -> anyhow::Result<Vec<Suggestion>> {
    ...
    let url = format!(
        "https://store.steampowered.com/search/suggest?cc=BR&l=brazilian&realm=1&origin=https:%2F%2Fstore.steampowered.com&f=jsonfull&term={encoded_term}&require_type=game,software"
    );
